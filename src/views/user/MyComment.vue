...
export default {
  data () {
    return {
      ...
      // 每页的条数
      pageSize: 5,
      // 代表加载状态，如果loading为true代表List组件正在加载数据 onLoad不再触发
      // 当滚动到底部， 触发onLoad的时候，自动把loading改成true
      loading: false,
      // 代表是否结束， false代表还有数据 finished代表没有更多数据
      // finised默认为false，如果后台没有更多数据就需要把finised改成true
      finished: false,
      // 代表是否正在下拉刷新
      refreshing: false
    }
